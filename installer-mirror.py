...
def fetch_versions():
    ...
    with urllib.request.urlopen(version_manifest) as response:
        ...
        for version in versions:
            id = version["id"]
            type = version["type"]
            url = version["url"].replace("https://zkitefly.github.io/unlisted-versions-of-minecraft", "https://raw.gitcode.com/zkitefly/unlisted-versions-of-minecraft/raw/main")
            release_time = version["releaseTime"]
            versions_tree.insert("", "end", values=(id, type, url, release_time))

# 创建主窗口
root = tk.Tk()
root.title("unlisted-versions-of-minecraft 版本下载器 - v1.5")
